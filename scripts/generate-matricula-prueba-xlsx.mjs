 ...
 * Uso: node scripts/generate-matricula-prueba-xlsx.mjs [cantidad] [dniBase]
 */
...
const HEADERS = [
  'nombres', 'apellidos', 'dni', 'email', 'sexo', 'fecha_nac',
  'nivel', 'grado', 'seccion', 'anio_ingreso',
  'apoderado_nombres', 'apoderado_apellidos', 'apoderado_dni',
...
function generarFilas(count, base) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    filas.push([
      nombre, `${ape1} ${ape2}`, dni, '', esF ? 'F' : 'M',
      cfg.fechas[i % cfg.fechas.length], cfg.nivel,
      cfg.grados[i % cfg.grados.length], secciones[i % secciones.length], '2026',
      esF ? 'Carlos' : 'Maria', `${ape1} ${ape2.split(' ')[0]}`,
      String(90210001 + i).padStart(8, '0'),
      `987${String(200000 + i).slice(-6)}`, `apoderado.${dni}@email.com`,
    ]);
  }
  return filas;
}
