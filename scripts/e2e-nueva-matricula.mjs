...
const MATRICULAS = [
  {
    ...
    dni: '80345678',
    sexo: 'Masculino',
    telEmergencia: '987111222',
    apoderado: 'Carlos Vega Ramos',
    celular: '987111222',
    nivel: 'Primaria',
    grado: '4',
    seccion: 'A',
...
async function nuevaMatriculaDesdeConfirmacion(page) {
...
try {
  ...
  for (const data of MATRICULAS) {
    const res = await crearMatricula(page, data);
    resultados.push(res);
    console.log(`OK: ${res.nombre} (${res.dni}) -> ${res.codigo}`);
    if (data !== MATRICULAS[MATRICULAS.length - 1]) {
      await nuevaMatriculaDesdeConfirmacion(page);
    }
  }
} catch (err) {
...
if (resultados.length === MATRICULAS.length) {
